...
# 加载预训练的YOLOv11n模型
model = YOLO(r"D:\waibao\cancer_detect\ultralytics-main-cancer\runs\train4\weights\best.pt")

# 对指定的图像文件夹进行推理，并设置各种参数
results = model.predict(
    source=r"D:\WeChat\WeChat Files\wxid_irdzyttbtypt22\FileStorage\File\2025-04\avi\data_origin\1/1-4.avi",  # 数据来源，可以是文件夹、图片路径、视频、URL，或设备ID（如摄像头）
    conf=0.5,  # 置信度阈值
    iou=0.5,  # IoU 阈值
    imgsz=640,  # 图像大小
    half=False,  # 使用半精度推理
